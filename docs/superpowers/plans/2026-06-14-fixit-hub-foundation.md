...
## Post-Implementation Note (final review)

- **Canonical category vocabulary**: `technicians.ts`, `appliances.ts`, and `applianceSpecs.json` use these exact category strings as the matching key for `lookupApplianceSpec` and `matchTechnicians`: `AC & HVAC`, `Refrigerator`, `Washing Machine`, `Television`, `Oven`, `Water Heater`. The design spec's UI copy uses shorter display labels for some of these (`Fridge`, `TV`). Plans 2–6 must map any shorter UI display label to the canonical key above before calling the matching helpers — passing `"Fridge"` or `"TV"` directly will silently fall through to category-typical/no-match results (matching is case-insensitive but not alias-aware).
